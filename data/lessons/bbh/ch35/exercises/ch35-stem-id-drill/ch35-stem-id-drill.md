# Chapter 35 — Stem Identification Drill (Weak Roots)

*BBH Chapter 35 · Hithpael Weak Verbs*

---

## Instructions

Identify the **stem**, **conjugation**, **person/gender/number**, and **root** of each form below. All forms use weak roots: **גלה** (III-ה), **הלל** (geminate), and **ענה** (III-ה).

Stems covered: Qal, Piel, Hithpael (8 items each, 24 total).

> **Key challenge:** For III-ה roots, the ה ending and vowel patterns change with conjugation. In wayyiqtol, the ה is dropped (apocopation). In imperfect and participle, R3-ה appears as Seghol + ה. In perfect, it appears as Qamets + ה.

Write your answers in the blank columns.

---

## Weak Pattern Reference

| Conjugation | III-ה Qal marker | III-ה Piel marker | III-ה Hithpael marker |
|---|---|---|---|
| Perfect 3ms | Qamets + ה (גָּלָה) | Dagesh R2 + Qamets + ה (גִּלָּה) | הִתְ prefix + Dagesh R2 + Qamets + ה |
| Imperfect 3ms | Seghol + ה (יִגְלֶה) | Shewa prefix + Seghol + ה (יְגַלֶּה) | יִתְ prefix + Dagesh R2 + Seghol + ה |
| Wayyiqtol 3ms | Apocopated: ה dropped (וַיִּגֶּל) | Apocopated: ה dropped (וַיְגַל) | Apocopated: ה dropped (וַיִּתְגַּל) |
| Imperative 2ms | Seghol + ה (גְּלֵה) | Pathach + Dagesh R2 + Seghol + ה (גַּלֵּה) | הִתְ prefix + Dagesh R2 + Seghol + ה |
| Participle ms | Qamets-Holem + ה (גֹּלֶה) | Shewa prefix + Seghol + ה (מְגַלֶּה) | מִתְ prefix + Dagesh R2 + Seghol + ה |

---

## Exercise Table

| # | Hebrew | Stem | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 1 | גָּלָה | | | | |
| 2 | גִּלָּה | | | | |
| 3 | הִתְגַּלָּה | | | | |
| 4 | הָלַל | | | | |
| 5 | הִלֵּל | | | | |
| 6 | הִתְהַלֵּל | | | | |
| 7 | עָנָה | | | | |
| 8 | עִנָּה | | | | |
| 9 | הִתְעַנָּה | | | | |
| 10 | יְגַלֶּה | | | | |
| 11 | יִגְלֶה | | | | |
| 12 | יִתְגַּלֶּה | | | | |
| 13 | יְהַלֵּל | | | | |
| 14 | יִתְהַלֵּל | | | | |
| 15 | יְעַנֶּה | | | | |
| 16 | וַיְגַל | | | | |
| 17 | וַיִּגֶּל | | | | |
| 18 | וַיִּתְגַּל | | | | |
| 19 | גַּלֵּה | | | | |
| 20 | גְּלֵה | | | | |
| 21 | הִתְגַּלֵּה | | | | |
| 22 | מְגַלֶּה | | | | |
| 23 | גֹּלֶה | | | | |
| 24 | מִתְגַּלֶּה | | | | |

---

## Answer Key

| # | Hebrew | Stem | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 1 | גָּלָה | **Qal** | Perfect | 3ms | גלה |
| 2 | גִּלָּה | **Piel** | Perfect | 3ms | גלה |
| 3 | הִתְגַּלָּה | **Hithpael** | Perfect | 3ms | גלה |
| 4 | הָלַל | **Qal** | Perfect | 3ms | הלל |
| 5 | הִלֵּל | **Piel** | Perfect | 3ms | הלל |
| 6 | הִתְהַלֵּל | **Hithpael** | Perfect | 3ms | הלל |
| 7 | עָנָה | **Qal** | Perfect | 3ms | ענה |
| 8 | עִנָּה | **Piel** | Perfect | 3ms | ענה |
| 9 | הִתְעַנָּה | **Hithpael** | Perfect | 3ms | ענה |
| 10 | יְגַלֶּה | **Piel** | Imperfect | 3ms | גלה |
| 11 | יִגְלֶה | **Qal** | Imperfect | 3ms | גלה |
| 12 | יִתְגַּלֶּה | **Hithpael** | Imperfect | 3ms | גלה |
| 13 | יְהַלֵּל | **Piel** | Imperfect | 3ms | הלל |
| 14 | יִתְהַלֵּל | **Hithpael** | Imperfect | 3ms | הלל |
| 15 | יְעַנֶּה | **Piel** | Imperfect | 3ms | ענה |
| 16 | וַיְגַל | **Piel** | Wayyiqtol | 3ms | גלה |
| 17 | וַיִּגֶּל | **Qal** | Wayyiqtol | 3ms | גלה |
| 18 | וַיִּתְגַּל | **Hithpael** | Wayyiqtol | 3ms | גלה |
| 19 | גַּלֵּה | **Piel** | Imperative | 2ms | גלה |
| 20 | גְּלֵה | **Qal** | Imperative | 2ms | גלה |
| 21 | הִתְגַּלֵּה | **Hithpael** | Imperative | 2ms | גלה |
| 22 | מְגַלֶּה | **Piel** | Participle | ms | גלה |
| 23 | גֹּלֶה | **Qal** | Participle | ms | גלה |
| 24 | מִתְגַּלֶּה | **Hithpael** | Participle | ms | גלה |

---

### Notes on Key Items

**Items 1–9 (Perfect 3ms):**
- Qal גָּלָה: Qamets under R1, no Dagesh in R2, Qamets + ה ending → Qal
- Piel גִּלָּה: Hireq under R1, Dagesh in R2 (לּ), Qamets + ה → Piel
- Hithpael הִתְגַּלָּה: הִתְ prefix, Dagesh in R2 (לּ), Qamets + ה → Hithpael
- Qal הָלַל: Qamets under R1, both ל letters present (no contraction in Qal perf. 3ms) → Qal geminate
- Piel הִלֵּל: הִ– looks like Hiphil/Hithpael prefix, but Dagesh in R2 (לּ) with Tsere → Piel; no ת after the prefix
- Hithpael הִתְהַלֵּל: הִתְ prefix present + Dagesh in R2/R3 (לּ) → Hithpael geminate

**Items 16–18 (Wayyiqtol — III-ה apocopation):**
- Piel וַיְגַל: וַ + יְ (Shewa, Piel prefix) → ה dropped; no final ה
- Qal וַיִּגֶּל: וַ + יִ (Hireq + Dagesh in yod) → ה dropped; Seghol visible
- Hithpael וַיִּתְגַּל: וַ + יִתְ + גַּל (apocopated) → ה dropped

**Items 19–21 (Imperative 2ms):**
- Piel גַּלֵּה: Pathach under R1, Dagesh in R2, Seghol + ה → Piel imp.
- Qal גְּלֵה: Shewa under R1, no Dagesh, Tsere + ה → Qal imp. (note: Tsere not Seghol)
- Hithpael הִתְגַּלֵּה: הִתְ prefix, Dagesh in R2, Tsere + ה → Hithpael imp. (note: Tsere + ה, same as Qal here; the הִתְ prefix is the decisive marker)

**Items 22–24 (Participle ms):**
- Piel מְגַלֶּה: מְ prefix (Shewa), Pathach R1, Dagesh R2, Seghol + ה → Piel ptc.
- Qal גֹּלֶה: Holem under R1 (Qal active ptc. pattern), no prefix → Qal
- Hithpael מִתְגַּלֶּה: מִתְ prefix (Hireq + ת), Dagesh R2, Seghol + ה → Hithpael ptc.

---

## Stem Distribution

| Stem | Items | Count |
|---|---|---|
| Qal | 1, 4, 7, 11, 17, 20, 23 | 7 |
| Piel | 2, 5, 8, 10, 13, 15, 16, 19, 22 | 9 |
| Hithpael | 3, 6, 9, 12, 14, 18, 21, 24 | 8 |
| **Total** | | **24** |
