# Stem Id Drill

*Chapter 28 — Hophal Strong*

[Full Screen (Interactive)](ch28-stem-id-drill.html){.md-button .md-button--primary}  [Print / PDF](ch28-stem-id-drill.pdf){.md-button}  [Markdown](ch28-stem-id-drill.md){.md-button}

*BBH Chapter 28 · Hophal Strong Verbs*

A 24-item identification drill mixing Qal, Niphal, Hiphil, and Hophal forms of strong roots (6 per stem), interleaved. Uses roots קטל (paradigm), שׁמר, כבד, and שׁמד. Students identify the stem, parse the conjugation and PGN, and translate each form. Items 16 and 20 share the identical spelling הַשְׁמֵר (Hiphil Imperative 2ms = Hiphil Inf. Absolute), reinforcing the ambiguity unique to strong roots. The Hophal has no Imperative; item 18 uses the Hophal Infinitive Construct in its place.

| File | Format |
|---|---|
| [ch28-stem-id-drill.md](ch28-stem-id-drill.md) | Static reference with answer key |
| [ch28-stem-id-drill.html](ch28-stem-id-drill.html) | Interactive (fillable, ▶ Answer per row) |
| [ch28-stem-id-drill.pdf](ch28-stem-id-drill.pdf) | Fillable PDF |

## Coverage

| File | Format |
|---|---|
| [ch28-stem-id-drill.md](ch28-stem-id-drill.md) | Static reference with answer key |
| [ch28-stem-id-drill.html](ch28-stem-id-drill.html) | Interactive (fillable, ▶ Answer per row) |
| [ch28-stem-id-drill.pdf](ch28-stem-id-drill.pdf) | Fillable PDF |

## Sample Questions

**Q1.** 1 קָטַל
> **A:** Qal Perfect 3ms קטל he killed

**Q2.** 2 הִקְטִיל
> **A:** Hiphil Perfect 3ms קטל he caused to kill · Hiphil marker: הִ prefix + chiriq in 2nd position (קְטִיל)

**Q3.** 3 נִקְטַל
> **A:** Niphal Perfect 3ms קטל he was killed · Niphal marker: נִ prefix in Perfect
