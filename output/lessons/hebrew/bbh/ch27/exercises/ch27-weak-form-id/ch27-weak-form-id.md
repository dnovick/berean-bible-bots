# "Weak-Form Identification Drill" — Exercise

*BBH Chapter 27 · Hiphil Weak Verbs*

---

## Instructions

**Part A:** Forms are grouped by weak class (5 per class, 8 groups = 40 items total). For each, identify: conjugation, PGN, root.
**Part B:** Forms are mixed. Identify: weak class first, then conjugation, PGN, root.

---

## Part A — By Class

### Group 1: I-guttural

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 1 | הֶעֱמִיד | 1 Kgs 7:21 | | | |
| 2 | וַיַּעֲמֵד | 2 Chr 4:4 | | | |
| 3 | יַעֲמִיד | Psa 107:29 | | | |
| 4 | הַעֲמֵד | Num 27:19 | | | |
| 5 | מַעֲמִיד | Neh 4:7 | | | |

### Group 2: III-ח/ע

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 6 | הִשְׁמִיעַ | Isa 48:6 | | | |
| 7 | וַיַּשְׁלַח | Gen 3:23 | | | |
| 8 | יַשְׁמִיעַ | Isa 42:2 | | | |
| 9 | הַשְׁמַע | Deu 4:10 | | | |
| 10 | מַשְׁמִיעַ | Isa 41:26 | | | |

### Group 3: III-א

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 11 | הִמְצִיא | Neh 9:15 | | | |
| 12 | וַיַּמְצֵא | 2 Chr 2:13 | | | |
| 13 | יַמְצִיא | Pro 8:35 | | | |
| 14 | וַיַּקְרֵא | Gen 1:5 | | | |
| 15 | מַקְרִיא | Neh 8:3 | | | |

### Group 4: III-ה

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 16 | הֶעֱלָה | Gen 8:20 | | | |
| 17 | וַיַּעַל | Gen 22:2 | | | |
| 18 | יַעֲלֶה | Lev 14:20 | | | |
| 19 | הַרְאֵה | Exo 33:18 | | | |
| 20 | לְהַעֲלוֹת | Exo 3:17 | | | |

### Group 5: I-נ

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 21 | הִפִּיל | Gen 2:21 | | | |
| 22 | וַיַּפֵּל | Gen 2:21 | | | |
| 23 | יַפִּיל | Pro 19:15 | | | |
| 24 | הַגֵּשׁ | Gen 27:25 | | | |
| 25 | מַגִּישׁ | Mal 1:7 | | | |

### Group 6: I-י

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 26 | הוֹצִיא | Gen 15:7 | | | |
| 27 | וַיּוֹצֵא | Gen 1:12 | | | |
| 28 | יוֹרִיד | 1 Sam 2:6 | | | |
| 29 | הוֹרֵד | Gen 42:38 | | | |
| 30 | מוֹצִיא | Deu 8:14 | | | |

### Group 7: Biconsonantal

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 31 | הֵקִים | Gen 6:18 | | | |
| 32 | וַיָּקֶם | Gen 23:20 | | | |
| 33 | יָקִים | Deu 18:15 | | | |
| 34 | הָקֵם | Exo 40:2 | | | |
| 35 | מֵקִים | 1 Sam 2:8 | | | |

### Group 8: Geminate (Ayin-Doubled)

| # | Form | Reference | Conjugation | PGN | Root |
|---|---|---|---|---|---|
| 36 | הֵסֵב | 1 Kgs 21:4 | | | |
| 37 | וַיָּסֶב | Josh 6:14 | | | |
| 38 | יָסֵב | Isa 44:20 | | | |
| 39 | הָסֵב | 2 Sam 2:22 | | | |
| 40 | מֵסֵב | Jer 21:4 | | | |

---

## Part B — Mixed (identify weak class first)

| # | Form | Reference | Weak class | Conjugation | PGN | Root |
|---|---|---|---|---|---|---|
| 41 | וַיַּשְׁמַע | 1 Sam 15:14 | | | | |
| 42 | הֵשִׂים | Gen 45:9 | | | | |
| 43 | וַיַּעַל | Gen 22:2 | | | | |
| 44 | הִגִּישׁ | Amos 9:13 | | | | |
| 45 | הָסֵב | 2 Sam 5:23 | | | | |
| 46 | הֶרְאָה | Exo 25:9 | | | | |
| 47 | וַיּוֹרֶד | Gen 42:38 | | | | |
| 48 | מַעֲמִידִים | Neh 4:7 | | | | |
| 49 | הַמְצֵא | (†) | | | | |
| 50 | וָאָקִים | Exo 6:4 | | | | |

---

## Answer Key — Part A

| # | Form | Conjugation | PGN | Root | Diagnostic |
|---|---|---|---|---|---|
| 1 | הֶעֱמִיד | Perfect | 3ms | עָמַד | *Seghol* under הֶ + *hateph-seghol* under ע |
| 2 | וַיַּעֲמֵד | Wayyiqtol | 3ms | עָמַד | *Patach* prefix; *hateph-patach* under ע |
| 3 | יַעֲמִיד | Imperfect | 3ms | עָמַד | *Patach* prefix; composite shewa under ע; no dagesh |
| 4 | הַעֲמֵד | Imperative | 2ms | עָמַד | הַ + composite shewa under ע + *tsere* |
| 5 | מַעֲמִיד | Participle | ms | עָמַד | מַ + composite shewa under ע + *chiriq* |
| 6 | הִשְׁמִיעַ | Perfect | 3ms | שָׁמַע | *Patach furtive* before final ע in 3ms |
| 7 | וַיַּשְׁלַח | Wayyiqtol | 3ms | שָׁלַח | *Patach* (not *tsere*) before final ח — guttural lowers vowel |
| 8 | יַשְׁמִיעַ | Imperfect | 3ms | שָׁמַע | *Patach furtive* before final ע |
| 9 | הַשְׁמַע | Imperative | 2ms | שָׁמַע | *Patach* before word-final ע (not *tsere*) |
| 10 | מַשְׁמִיעַ | Participle | ms | שָׁמַע | מַ prefix + *patach furtive* before final ע |
| 11 | הִמְצִיא | Perfect | 3ms | מָצָא | *Chiriq-yod* + silent final א |
| 12 | וַיַּמְצֵא | Wayyiqtol | 3ms | מָצָא | *Tsere* + silent final א |
| 13 | יַמְצִיא | Imperfect | 3ms | מָצָא | *Chiriq-yod* + silent final א |
| 14 | וַיַּקְרֵא | Wayyiqtol | 3ms | קָרָא | *Tsere* + silent final א |
| 15 | מַקְרִיא | Participle | ms | קָרָא | מַ + *chiriq-yod* + silent final א |
| 16 | הֶעֱלָה | Perfect | 3ms | עָלָה | *Qamets* + ה ending; also *seghol* under הֶ (I-gutt.) |
| 17 | וַיַּעַל | Wayyiqtol | 3ms | עָלָה | Apocopated — ה dropped; short *patach* under R2 |
| 18 | יַעֲלֶה | Imperfect | 3ms | עָלָה | *Seghol* + ה ending |
| 19 | הַרְאֵה | Imperative | 2ms | רָאָה | *Tsere* + ה ending (ה retained in imperative) |
| 20 | לְהַעֲלוֹת | Inf. Construct | — | עָלָה | Ends in וֹת — strong III-ה Hiphil inf. construct marker |
| 21 | הִפִּיל | Perfect | 3ms | נָפַל | *Dagesh forte* in R2 (פ) — נ has assimilated |
| 22 | וַיַּפֵּל | Wayyiqtol | 3ms | נָפַל | *Patach* prefix + *dagesh* in פ + *tsere* |
| 23 | יַפִּיל | Imperfect | 3ms | נָפַל | *Patach* prefix + *dagesh* in R2 + *chiriq* |
| 24 | הַגֵּשׁ | Imperative | 2ms | נָגַשׁ | הַ + *dagesh forte* in R2 (ג) + *tsere* |
| 25 | מַגִּישׁ | Participle | ms | נָגַשׁ | מַ + *dagesh* in R2 + *chiriq* |
| 26 | הוֹצִיא | Perfect | 3ms | יָצָא | הוֹ prefix (*holem-vav*) — classic I-י/ו marker |
| 27 | וַיּוֹצֵא | Wayyiqtol | 3ms | יָצָא | וַיּוֹ prefix — dagesh in יּ + *holem-vav* |
| 28 | יוֹרִיד | Imperfect | 3ms | יָרַד | יוֹ prefix (*holem-vav*) |
| 29 | הוֹרֵד | Imperative | 2ms | יָרַד | הוֹ prefix + *tsere* (not הַ + *tsere*) |
| 30 | מוֹצִיא | Participle | ms | יָצָא | מוֹ prefix (*holem-vav*) — not מַ |
| 31 | הֵקִים | Perfect | 3ms | קוּם | הֵ prefix (*tsere*) — not הִ (*hiriq*) |
| 32 | וַיָּקֶם | Wayyiqtol | 3ms | קוּם | *Qamets* under prefix + apocopated *seghol* final |
| 33 | יָקִים | Imperfect | 3ms | קוּם | *Qamets* under prefix (not *patach*) |
| 34 | הָקֵם | Imperative | 2ms | קוּם | הָ prefix (*qamets*) + *tsere* |
| 35 | מֵקִים | Participle | ms | קוּם | מֵ prefix (*tsere*) — not מַ |
| 36 | הֵסֵב | Perfect | 3ms | סָבַב | הֵ prefix (*tsere*) — same as Biconsonantal הֵקִים; root is ס-ב-ב (Geminate) |
| 37 | וַיָּסֶב | Wayyiqtol | 3ms | סָבַב | *Qamets* prefix + contracted root + *seghol* — same pattern as Biconsonantal וַיָּקֶם |
| 38 | יָסֵב | Imperfect | 3ms | סָבַב | *Qamets* under prefix (יָ) — same as Biconsonantal יָקִים |
| 39 | הָסֵב | Imperative | 2ms | סָבַב | הָ prefix (*qamets*) — same as Biconsonantal הָקֵם |
| 40 | מֵסֵב | Participle | ms | סָבַב | מֵ prefix (*tsere*) — same as Biconsonantal מֵקִים |

---

## Answer Key — Part B

| # | Form | Weak class | Conjugation | PGN | Root | Note |
|---|---|---|---|---|---|---|
| 41 | וַיַּשְׁמַע | III-ח/ע | Wayyiqtol | 3ms | שָׁמַע | *Patach* before final ע (not *tsere*) |
| 42 | הֵשִׂים | Biconsonantal | Perfect | 3ms | שִׂים | הֵ prefix (*tsere*); root has medial *hireq-yod* vowel letter |
| 43 | וַיַּעַל | III-ה | Wayyiqtol | 3ms | עָלָה | Apocopated — ה dropped |
| 44 | הִגִּישׁ | I-נ | Perfect | 3ms | נָגַשׁ | *Dagesh* in R2 (ג); root נגשׁ |
| 45 | הָסֵב | Geminate | Imperative | 2ms | סָבַב | הָ prefix (*qamets*) — looks like Biconsonantal; root ס-ב-ב has R2=R3 (Geminate) |
| 46 | הֶרְאָה | III-ה | Perfect | 3ms | רָאָה | *Qamets* + ה ending; *seghol* under הֶ |
| 47 | וַיּוֹרֶד | I-י | Wayyiqtol | 3ms | יָרַד | וַיּוֹ prefix |
| 48 | מַעֲמִידִים | I-guttural | Participle | mp | עָמַד | מַ + composite shewa under ע; plural |
| 49 | הַמְצֵא | III-א | Imperative | 2ms | מָצָא | *Tsere* + silent final א |
| 50 | וָאָקִים | Biconsonantal | Wayyiqtol | 1cs | קוּם | וָאָ prefix + *chiriq-yod* medial; medial ו/י vowel letter = Biconsonantal |

---

## Discussion Questions

**1.** Compare הִמְצִיא (III-א perfect 3ms) and הַמְצֵא (III-א imperative 2ms). Both end with silent final א. How do you distinguish them? What is the key difference in the prefix vowel?

**2.** הַשְׁמַע (III-ח/ע imperative) and הַעֲמֵד (I-guttural imperative) both begin with הַ. How does the vowel under R1 differ? What does that tell you about the class?

**3.** וַיַּשְׁלַח (III-ח/ע wayyiqtol) and וַיַּקְרֵא (III-א wayyiqtol) both have *patach* under וַיַּ. The difference is in the final vowel. Explain what happens to the Hiphil *tsere* in each case and why.

**4.** Compare הוֹרֵד (I-י imperative 2ms) and הָקֵם (Biconsonantal imperative 2ms). Both have a *long* prefix vowel rather than the *patach* (הַ) of the strong Hiphil imperative. What prefix vowel does each use, and how can you tell them apart?

**5.** Items 36–40 (Geminate) and items 31–35 (Biconsonantal) share nearly identical vowel patterns in every conjugation: הֵ perfect, יָ/תָּ imperfect, הָ imperative, מֵ participle. What is the **only** reliable way to determine whether a Hiphil form belongs to the Geminate class or the Biconsonantal class? Why can the vocalization alone not always resolve this question?
